...
describe('verifyRefreshToken', () => {
  ...
  test('throws Unauthorized when jwt throws', async () => {
    ...
    await expect(authService.verifyRefreshToken('bad')).rejects.toThrow('Unauthorized');
  });

  test('throws Unauthorized when user not found', async () => {
    jwt.verify.mockReturnValueOnce({ id: '1' });
    User.findById.mockResolvedValue(null);
    await expect(authService.verifyRefreshToken('tok')).rejects.toThrow('Unauthorized');
  });
...
describe('loginUser', () => {
  test('returns user on valid credentials and verified email', async () => {
    const user = mkUser({ isEmailVerified: true, matchPassword: jest.fn().mockResolvedValue(true) });
  ...
  test('throws 401 when user not found', async () => {
    ...
    await expect(authService.loginUser('x@x.com', 'p')).rejects.toThrow('Invalid email or password.');
  });

  test('throws 401 when password wrong', async () => {
    const user = mkUser({ matchPassword: jest.fn().mockResolvedValue(false) });
    User.findOne.mockReturnValue({ select: jest.fn().mockResolvedValue(user) });
    await expect(authService.loginUser('dj@beats.com', 'wrong')).rejects.toThrow('Invalid email or password.');
  });

  test('throws 403 when email not verified', async () => {
    const user = mkUser({ isEmailVerified: false, matchPassword: jest.fn().mockResolvedValue(true) });
    User.findOne.mockReturnValue({ select: jest.fn().mockResolvedValue(user) });
    await expect(authService.loginUser('dj@beats.com', 'pass')).rejects.toThrow('Please verify your email');
  });
});
...
describe('registerUser', () => {
  test('throws 400 when no captcha', async () => {
    await expect(authService.registerUser({}, null)).rejects.toThrow('CAPTCHA token is required.');
  });

  test('throws 400 when captcha fails', async () => {
    axios.post.mockResolvedValue({ data: { success: false } });
    await expect(authService.registerUser({}, 'bad')).rejects.toThrow('CAPTCHA verification failed');
  });

  test('throws 409 when email taken', async () => {
    axios.post.mockResolvedValue({ data: { success: true } });
    User.findOne.mockResolvedValue(mkUser());
    await expect(authService.registerUser({ email: 'dj@beats.com' }, 'tok')).rejects.toThrow('Email is already registered.');
  });
  ...
  test('creates user and sends verification email', async () => {
    ...
    const r = await authService.registerUser({ email: 'new@beats.com', displayName: 'New' }, 'valid');
  ...
  test('silently continues if email send fails', async () => {
    ...
    await expect(authService.registerUser({ email: 'x@x.com' }, 'v')).resolves.toBeDefined();
  });
});
...
describe('generatePasswordReset', () => {
  ...
  test('throws 404 when user not found', async () => {
    User.findOne.mockResolvedValue(null);
    await expect(authService.generatePasswordReset('no@user.com')).rejects.toThrow('No user found with that email.');
  });
  ...
  test('clears token and throws 500 if email fails', async () => {
    ...
    crypto.randomBytes.mockReturnValue({ toString: () => 'rtok' });
    sendEmail.mockRejectedValue(new Error('SMTP'));
    await expect(authService.generatePasswordReset('dj@beats.com')).rejects.toThrow('Email could not be sent');
...
describe('resetPassword', () => {
  ...
  test('throws 400 on invalid token', async () => {
    User.findOne.mockResolvedValue(null);
    await expect(authService.resetPassword('bad', 'pass')).rejects.toThrow('Invalid or expired password reset token.');
  });
});
...
describe('logoutUser', () => {
  test('clears refresh token in DB', async () => {
    ...
    expect(r).toBe(true);
    expect(User.findByIdAndUpdate).toHaveBeenCalledWith('uid', { refreshToken: null });
  });
});
...
describe('resendVerificationEmail', () => {
  test('returns silently when user not found', async () => {
    User.findOne.mockResolvedValue(null);
    await expect(authService.resendVerificationEmail('x@x.com')).resolves.toBeUndefined();
    expect(sendEmail).not.toHaveBeenCalled();
  });

  test('returns silently when already verified', async () => {
    User.findOne.mockResolvedValue(mkUser({ isEmailVerified: true }));
    await expect(authService.resendVerificationEmail('dj@beats.com')).resolves.toBeUndefined();
    expect(sendEmail).not.toHaveBeenCalled();
  });
...
describe('requestEmailUpdate', () => {
  test('throws 409 when new email already taken', async () => {
    User.findOne.mockResolvedValue(mkUser());
    await expect(authService.requestEmailUpdate('uid', 'taken@email.com')).rejects.toThrow('already registered');
  });

  test('throws 404 when user not found', async () => {
    User.findOne.mockResolvedValue(null);
    User.findById.mockResolvedValue(null);
    await expect(authService.requestEmailUpdate('uid', 'new@email.com')).rejects.toThrow('User not found.');
  });
  ...
  test('stores pending email and sends confirmation', async () => {
    ...
    expect(sendEmail).toHaveBeenCalledWith(expect.objectContaining({ email: 'new@email.com' }));
  });
});
...
describe('confirmEmailUpdate', () => {
  test('applies email change on valid token', async () => {
    const user = mkUser({ pendingEmail: 'new@email.com', pendingEmailToken: 'etok' });
    User.findOne.mockResolvedValue(user);
  ...
  test('throws 400 on invalid token', async () => {
    User.findOne.mockResolvedValue(null);
    await expect(authService.confirmEmailUpdate('bad')).rejects.toThrow('Invalid or expired email update token.');
  });

  test('throws 400 when pendingEmail missing', async () => {
    User.findOne.mockResolvedValue(mkUser({ pendingEmail: null, pendingEmailToken: 'tok' }));
    await expect(authService.confirmEmailUpdate('tok')).rejects.toThrow('Invalid or expired email update token.');
  });
});
