...
const mkChain = (resolved) => {
  ...
  // select returns a promise that also has .lean() for the lean() path
  const selectResult = Object.assign(Promise.resolve(resolved), {
    lean: obj.lean,
  });
  obj.select = jest.fn().mockReturnValue(selectResult);
  return obj;
};
...
describe('profileService.getProfileByPermalink', () => {
  ...
  test('throws 404 when not found', async () => {
    ...
    await expect(
      profileService.getProfileByPermalink('nobody')
...
describe('profileService.updatePrivacy', () => {
  test('updates isPrivate field', async () => {
    ...
    expect((await profileService.updatePrivacy(UID, true)).isPrivate).toBe(
      true
    );
  });
  ...
  test('throws when user not found', async () => {
    ...
    await expect(profileService.updatePrivacy('bad', false)).rejects.toThrow(
      'User not found'
    );
  });
});
...
describe('profileService.updateSocialLinks', () => {
  ...
  test('updates links when changed', async () => {
    const user = { socialLinks: [{ platform: 'TW', url: 'https://tw.com' }] };
    User.findById.mockReturnValue(mkChain(user));
    User.findByIdAndUpdate.mockReturnValue(mkChain({ socialLinks: newLinks }));
  ...
  test('throws 404 when user not found', async () => {
    ...
    await expect(
      profileService.updateSocialLinks(UID, newLinks)
...
describe('profileService.removeSocialLink', () => {
  ...
  test('throws 404 when link not found', async () => {
    ...
    User.findById.mockReturnValue(mkChain(user));
    await expect(profileService.removeSocialLink(UID, 'bad')).rejects.toThrow(
  ...
  test('throws 404 when user not found', async () => {
    User.findById.mockReturnValue(mkChain(null));
    await expect(profileService.removeSocialLink(UID, 'link')).rejects.toThrow(
      'User not found'
    );
  });
});
...
describe('profileService.updateTier', () => {
  test('updates role', async () => {
    ...
    expect((await profileService.updateTier(UID, 'Artist')).role).toBe(
      'Artist'
    );
  });
  ...
  test('throws when user not found', async () => {
    User.findByIdAndUpdate.mockReturnValue(mkChain(null));
    await expect(profileService.updateTier('bad', 'Artist')).rejects.toThrow(
      'User not found'
    );
  });
});
...
describe('profileService.updateProfileImages', () => {
  test('uploads avatar to Azure', async () => {
    ...
    User.findByIdAndUpdate.mockReturnValue(
      mkChain({ avatarUrl: 'https://blob/av.png' })
    );
    const r = await profileService.updateProfileImages(UID, {
      avatar: [{ buffer: Buffer.from('x'), mimetype: 'image/jpeg' }],
    });
  ...
  test('uploads cover to Azure', async () => {
    ...
    User.findByIdAndUpdate.mockReturnValue(
      mkChain({ coverUrl: 'https://blob/co.png' })
    );
    await profileService.updateProfileImages(UID, {
      cover: [{ buffer: Buffer.from('x'), mimetype: 'image/png' }],
    });
    expect(uploadImageToAzure).toHaveBeenCalledWith(
      expect.any(Buffer),
      'image/png',
  ...
  test('throws when no valid files', async () => {
    await expect(profileService.updateProfileImages(UID, {})).rejects.toThrow(
      'No valid image fields provided'
    );
  });
});
...
describe('networkService.followUser', () => {
  ...
  test('throws when following self', async () => {
    await expect(networkService.followUser(UID, UID)).rejects.toThrow(
  ...
  test('throws when target not found', async () => {
    ...
    await expect(networkService.followUser(UID, UID2)).rejects.toThrow(
      'User not found.'
    );
  });
...
describe('networkService.unfollowUser', () => {
  ...
  test('throws when not following', async () => {
    ...
    await expect(networkService.unfollowUser(UID, UID2)).rejects.toThrow(
      'not following'
    );
  });
});
...
describe('networkService.getFollowers', () => {
  test('returns follower list', async () => {
    Follow.find.mockReturnValue(
      mkFollowChain([{ follower: { displayName: 'Fan' } }])
    );
...
describe('networkService.getFollowing', () => {
  test('returns following list', async () => {
    Follow.find.mockReturnValue(
      mkFollowChain([{ following: { displayName: 'Artist' } }])
    );
...
describe('networkService.getSuggestedUsers', () => {
  ...
  test('returns popular fallback when user follows nobody', async () => {
    ...
    User.find.mockReturnValue(
      mkUserFindChain([{ _id: UID2, displayName: 'Popular' }])
    );
  ...
  test('uses aggregate for mutual follows then popularity fallback', async () => {
    ...
    Block.find.mockResolvedValue([]);
    Follow.aggregate = jest
      .fn()
      .mockResolvedValue([{ _id: mutualId, mutualCount: 2 }]);
    // User.find for mutual users needs .select() chain; fallback also needs full chain
    const mkSelectChain = (data) => ({
      select: jest.fn().mockResolvedValue(data),
    });
    User.find
      .mockReturnValueOnce(
        mkSelectChain([{ _id: mutualId, displayName: 'Mutual' }])
      )
...
describe('networkService.getBlockedUsers', () => {
  test('returns blocked users', async () => {
    Block.find.mockReturnValue({
      ...
      sort: jest
        .fn()
        .mockResolvedValue([{ blocked: { displayName: 'Blocked' } }]),
    });
...
describe('networkService.blockUser', () => {
  ...
  test('throws 400 when blocking self', async () => {
    await expect(networkService.blockUser(UID, UID)).rejects.toThrow(
  ...
  test('throws 409 when already blocked', async () => {
    ...
    await expect(networkService.blockUser(UID, UID2)).rejects.toThrow(
      'already blocked'
    );
  });
});
...
describe('networkService.unblockUser', () => {
  test('unblocks user', async () => {
    Block.findOneAndDelete.mockResolvedValue({ _id: 'blkid' });
    expect((await networkService.unblockUser(UID, UID2)).status).toBe(
      'unblocked'
    );
  });
  ...
  test('throws 404 when not blocked', async () => {
    ...
    await expect(networkService.unblockUser(UID, UID2)).rejects.toThrow(
      'User is not blocked'
    );
  });
});
